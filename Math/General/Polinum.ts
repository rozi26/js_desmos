...
export class Polinum<T extends NumberBase<T>>
{
    cofs: T[]
    constructor(cofs: T[] = [])
    {
    ...
    add(pol: Polinum<T>)
    {
        ...
        for (let i = 0; i < mm; i++) this.cofs[i].add(pol.cofs[i]);
        if (mm == pol.cofs.length) return;
        for (let i = mm; i < pol.cofs.length; i++) this.cofs.push(pol.cofs[i]);
    }

    mul(pol: Polinum<T>) //for now slow n^2 methods
    {
        const res: T[] = []
        for (let i = 1; i < this.cofs.length + pol.cofs.length; i++) res.push(this.cofs[0].fromNumber(0));
        let temp: T;
