...
import { Desmos2DPoint } from "../../Desmos/2D/Elements/Point/Desmos2DPoint.js";
...
{
    displayLayer: NewtonRafsonLayer
    pointsDisplayers: Point2DLayer[]

    constructor(writer: CanvasWriterPlus)
    {
        super(writer,Transformer2DLinearCreator);
        const defultPoints: ComplexNum[] = [];
        const amountOfDefult = 3;
        for (let i = 0; i < amountOfDefult; i++) defultPoints.push(new ComplexNum(Math.cos(Math.PI*2*i/amountOfDefult),Math.sin(Math.PI*2*i/amountOfDefult)));
