...
function PreProsses(text) {
    ...
    for (let i = 1; i < text.length; i++) {
        if (isLetter(text[i]) && isDigit(text[i - 1]))
    ...
    for (let p = 0; p < openOpers.length; p++) {
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (i1 === undefined || i2 === undefined)
                throw "fail to find expersion bounds in preprosses";
...
function cleanParseMath(text, funcs) {
    ...
    const innerExprs = [];
    for (let i = 0; i < funcVars.length; i++)
        innerExprs.push(cleanParseMath(funcVars[i], funcs));
    let maxInput = 0;
    for (let i = 0; i < funcVars.length; i++)
        maxInput = Math.max(maxInput, innerExprs[i].inpSize);
    const func = funcs.getOpe(funcName, funcVars.length);
    if (func === undefined)
        throw `couldn't find function named [${funcName}] that get [${funcVars.length}] inputs`;
    if (maxInput === 0) //calculate the result of the function in complation time so make the method faster
     {
        const inps = [];
        for (let i = 0; i < innerExprs.length; i++)
            inps[i] = innerExprs[i].calc();
    ...
    return new MathExpr((parms) => {
        const inps = [];
        for (let i = 0; i < innerExprs.length; i++)
            inps[i] = innerExprs[i].calc(parms);
        return func.calc(inps);
    }, maxInput);
}
export function ParseMath(text, funcs) {
    text = PreProsses(text);
    return cleanParseMath(text, funcs);
}
export function GetMathVars(text, funcs) {
    ...
    for (let i = 0; i < sentances.length; i++) {
        if (i == 0 || sentances[i - 1] !== sentances[i])
            setSentances.push(sentances[i]);
    }
...
export function ParseMathVars(text, funcs) {
    const vars = GetMathVars(text, funcs);
    if (vars.length == 0)
        return ParseMath(text, funcs);
...
//# sourceMappingURL=Parser1.js.map